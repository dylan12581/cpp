...
int main(){
    ...
    //FD_SETSIZE定义在/usr/include/linux/posix_types.h //1024
    int client[FD_SETSIZE];
    char buf[BUFSIZ], str[BUFSIZ];
    //1、创建套接字
    listenfd = socket(AF_INET, SOCK_STREAM, 0);
    int flag = fcntl(listenfd,F_GETFD);// 非阻塞socket
    fcntl(listenfd,flag | FD_CLOEXEC);
    ...
    while(1){
        ...
        if(FD_ISSET(listenfd, &rset)){
            ...
            //建立连接的文件描述符,表明三次握手建立成功的连接的文件描述符
            for(i = 0; i < FD_SETSIZE; ++i){
                if(client[i] < 0){
                    client[i] = connfd;
                    break;
            ...
            //将建立了三次握手的文件描述符放在allset集合中进行继续监听，如果该文件描述符后续继续可读，表明
            //有数据需要进行读写
